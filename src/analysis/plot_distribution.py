...
with plt.style.context(('seaborn-darkgrid')):
    ...
    # plt.title('Distribuição das médias das avaliações - Dataset Completo')
    plt.ylabel('Quantidade')
    plt.xlabel('Nota')
    plt.xlim(2,7)
    plt.hist(imgs_csv['mean_ratings'].values, bins=25, histtype='stepfilled')
    plt.tight_layout()
    plt.savefig('experiments/distribution_complete.pdf')
...
with plt.style.context(('seaborn-darkgrid')):
    ...
    # plt.title('Distribuição das médias das avaliações - Dataset Treinamento')
    plt.ylabel('Quantidade')
    plt.xlabel('Nota')
    plt.xlim(2,7)
    plt.hist(imgs_train['mean_ratings'].values, bins=25, histtype='stepfilled')
    plt.tight_layout()
    plt.savefig('experiments/distribution_train.pdf')

with plt.style.context(('seaborn-darkgrid')):
    f = plt.figure(figsize=(5.5,4))
    
    # plt.title('Distribuição das médias das avaliações - Dataset Teste')
    plt.ylabel('Quantidade')
    plt.xlabel('Nota')
    plt.xlim(2,7)
    plt.hist(imgs_test['mean_ratings'].values, bins=25, histtype='stepfilled')
    plt.tight_layout()
    plt.savefig('experiments/distribution_test.pdf')